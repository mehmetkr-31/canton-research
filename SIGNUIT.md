...
├──────────────────────────┬───────────────────────────────────┤
│   Daml Smart Contracts   │    Data Layer                    │
│                          │                                   │
│  - CollateralPolicy.daml│  ┌────────────────────────────┐  │
│  - RoutingSuggestion.daml│  │ Mock JSON (MVP)             │  │
│  - AllocationRecord.daml│  │ Postgres if needed for        │  │
│                          │  │ history/search only         │  │
│                          │  └────────────────────────────┘  │
├──────────────────────────┴───────────────────────────────────┤
│                      Canton Network                          │
...
**Note:** This is a minimal stack. All external integrations (Chainlink, Circle, DTCC) are mocked for MVP. Only Canton ledger integration is required — policy storage and audit record creation.
...
- They're already managing manually with spreadsheets — pain is known and admitted
...
## MVP Roadmap (HackCanton) — Super-Cut Build

### The Simplest Honest Framing

> SignUIT recommends which collateral to route for a single margin call, shows why, and records the decision on Canton.

### 48-Hour Build Plan

**Critical Path (6 items in order):**

1. **Policy Setup UI** — Simple form: priority order (e.g. USYC → UST → USDC), min LTV, max haircut
2. **Holdings Input Table** — Mock table: asset, amount, yield, haircut, expiry, eligible (Y/N)
3. **Routing Decision Function** — Basic rule engine: check eligible → sort by policy → pick best → calculate score → show savings
4. **Suggestion Screen** — Display: margin call amount → recommended asset(s) → why chosen → estimated savings
5. **Approve Button** — User approves/rejects the routing decision
6. **Ledger Write** — On approval: create Canton record with decision, timestamp, approver

**Daml Templates (2–3 only):**
- `CollateralPolicy` — policy and rule storage
- `RoutingSuggestion` — pending recommendation
- `AllocationRecord` — immutable audit log
...
Min 2–4: LIVE DEMO (single margin call scenario)
  1. Margin call received: $15M required
  2. Signuit shows recommendation with CTD rationale:
     - USYC first (lowest yield loss)
     - UST second
     - USDC last resort
  3. User clicks Approve
  4. Canton stores routing decision
  5. Audit trail shows: policy used → asset chosen → timestamp → approved by

Min 4–5: MARKET + ASK
  "We help institutions choose the right collateral for a margin call
  according to policy, and record that choice privately on Canton."
```

### What to Cut (Explicit)

- Weekend automation
- Yield Maximizer rule
- Cross-currency / FX routing
- Cross-border substitution
- Live Chainlink oracle feeds
- Tokenized Treasury integration
- Actual payment/settlement execution
- Multi-institution dashboards
- Enterprise analytics
- Complex CTD optimization

### What to Mock (Intentional, Not Shortcut)

- Asset prices (static mock values)
- Haircut schedules
- Eligibility rules
- Margin call events
- Savings calculations

This is intentional — the intent is to prove the core insight:
> Given a policy and eligible holdings, Signuit can recommend the right asset and record the decision privately on Canton.

That's enough to validate demand, not execution.

---

> **Phase 2 / post-hackathon:**
> - Weekend automation (James journey)
> - Cross-border substitution (Ana journey)
...
- One rule type: Cheapest-to-Deliver
- Three mock assets: USDC, USYC, US Treasury token
- One counterparty rule
- Policy suggestion with approval workflow
- Canton ledger record creation (single institution, single counterparty)
- Audit trail display
- Canton testnet deployment
- Demo account with pre-loaded holdings

### MVP Scope (Out — Post-Hackathon)

All items moved to Phase 2 list above.

---

### Hard Parts & Risk Table

The mentor identified five hard parts. Here's the risk breakdown:

| Hard Part | Risk | Severity | Mitigation |
|---|---|---|---|
| Correct collateral selection logic | Bad recommendation / false savings claim | High | Start with simple scoring: yield + haircut + eligibility. No fancy optimization in v1. |
| Integration with real assets | No execution path / fake spreadsheet UI | High | Use mock holdings for Demo. Show routing suggestion + audit record. Do not promise production settlement. |
| Canton contract design | Too many contracts / broken visibility | Medium | Keep to 3 templates: `CollateralPolicy`, `RoutingSuggestion`, `AllocationRecord`. |
| Privacy model | Leaking strategy / holdings / CTD policy | Medium | Single institution, single counterparty, one private route. Reduces complexity. |
| Execution trust | Low trust / manual override every time | Medium | UI explains: why asset chosen, what rule fired, estimated savings, what-if comparison. Transparency is key. |
...
> **"We help institutions choose the right collateral for a margin call according to policy, and record that choice privately on Canton."**

**SignUIT CollateralRouter** is a collateral operating system for Canton-native institutions. It helps them choose eligible collateral according to policy, with privacy and auditability built in.

**The core problem:** Canton has tokenized assets and private settlement rails, but no decision engine that suggests which collateral to route.

**The solution:** A policy suggestion engine where institutions define their rules once; SignUIT recommends the best collateral for each margin call and records the decision on Canton — Day 1 with human approval, automated execution opt-in for Phase 2.
...
> What evidence do we have that a real institution would trust SignUIT's recommendation instead of doing it in-house or inside an existing treasury system?

This is the most important question — not the business model, not the revenue projection, but the validation that institutions will actually use the suggestion engine.

**Set your policies. Get recommendations. Record decisions.**

---

*Document version: 1.3 — Super-cut MVP scope + risk table, HackCanton Season #1*
